...
CONFIG_FILE="$FUSION_HOME/conf/config.sh"
...
function check_java() {
  ...
  JAVA_VERSION=$($JAVA -version 2>&1 | awk '/version/{print $NF}' | sed 's/"//g' | sed 's/_.*//')
  ...
  if ! java -d64 -version >/dev/null 2>&1; then
    output "This product requires a 64-bit JVM"
    exit
  fi
}
...
function extra_java_options() {
  ...
  if [ $JAVA_VERSION = '1.7.0' ]; then
    ...
    PATCH_LEVEL=$($JAVA -version 2>&1 | awk '/version/{print $NF}' | sed 's/"//g' | sed 's/^.*_//')
  ...
  SCRIPT_DATETIME=$(date +"%Y%m%d_%H%M")
  JAVA_OPTIONS=("${JAVA_OPTIONS[@]}" "${GC_TUNE[@]}" "${GC_LOG_OPTS[@]}" "-Xloggc:${LOG_DIR}/gc_$SCRIPT_DATETIME.log")
  # strip empty options
