...
set "LOG_DIR=%FUSION_HOME%\logs\spark-worker"
set FUSION_SERVICE_NAME=Fusion Spark Worker
set APP_NAME=spark-worker
set "APP_DIR=%FUSION_HOME%\apps\spark-worker"
...
@REM --------------------------------------------------------------------------------------
:do_start
set "SPARK_WORKER_OPTS=!JAVA_OPTIONS! -Dcurator.zk.connect=%FUSION_ZK%  -Dlog4j.configurationFile=file:"%APP_HOME%\log4j2.xml"
...
set "SPARK_WORKER_OPTS=%SPARK_WORKER_OPTS%  -Dapollo.home=%FUSION_HOME% -Dspark.home=%SPARK_HOME%"

START "%FUSION_SERVICE_NAME%" /D"%FUSION_HOME%" "%APP_HOME%\bin\spark-worker.bat" %SPARK_WORKER_OPTS%
