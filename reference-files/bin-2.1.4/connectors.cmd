...
@REM --------------------------------------------------------------------------------------
set SERVICE_PORT=%CONNECTORS_PORT%
set /A STOP_PORT=%SERVICE_PORT% - 1000

set "JETTY_BASE=%FUSION_HOME%\apps\jetty\connectors"
set "LOG_DIR=%FUSION_HOME%\var\log\connectors"
set "VAR_DIR=%FUSION_HOME%\var\connectors"
set FUSION_SERVICE_NAME=Fusion Connectors
set APP_NAME=connectors
set "APP_DIR=%JETTY_BASE%\webapps\connectors"
...
@REM --------------------------------------------------------------------------------------
@REM Start the Fusion Connectors service
@REM --------------------------------------------------------------------------------------
:do_start
START "%FUSION_SERVICE_NAME%" /D"%FUSION_HOME%" "%JAVAW%" -cp "%FUSION_HOME%\connectors\conf" -server -d64 -Xss256k %JAVA_OPTIONS% ^
-Xloggc:"!LOG_DIR!"\gc_!SCRIPT_DATETIME!.log ^
-Djava.io.tmpdir="%VAR_DIR%\work" ^
-Dapollo.connectors.app="%FUSION_HOME%\apps\connectors" ^
-Dapollo.connectors.conf="%FUSION_HOME%\connectors/conf" ^
-Dapollo.connectors.data="%FUSION_HOME%\data\connectors" ^
-Dapollo.connectors.logs="%FUSION_HOME%\var\log\connectors" ^
