...
@REM --------------------------------------------------------------------------------------
set SERVICE_PORT=%SPARK_WORKER_PORT%
set "JAVA_OPTIONS=-Djava.net.preferIPv4Stack=true -Xmx512m -Dapple.awt.UIElement=true"

set "JETTY_BASE=%FUSION_HOME%\jetty\api"
set "LOG_DIR=%FUSION_HOME%\var\log\spark-worker"
set FUSION_SERVICE_NAME=Fusion Spark Worker
...
@REM --------------------------------------------------------------------------------------
@REM Start the Spark Worker
@REM --------------------------------------------------------------------------------------
:do_start
set SPARK_WORKER_OPTS=!JAVA_OPTIONS! -Dcurator.zk.connect=%FUSION_ZK% -Dlog4j.configurationFile=file:"%APP_DIR%"\conf\spark-worker-log4j2.xml
set SPARK_WORKER_OPTS=%SPARK_WORKER_OPTS% -Dfusion.spark.worker.port=%SPARK_WORKER_PORT% -Dfusion.spark.worker.ui.port=%SPARK_WORKER_UI_PORT%
set SPARK_WORKER_OPTS=%SPARK_WORKER_OPTS% -Dapollo.home="%FUSION_HOME%" -Dspark.home="%SPARK_HOME%" -Xloggc:"!LOG_DIR!"\gc_!SCRIPT_DATETIME!.log

START /B "%FUSION_SERVICE_NAME%" /D"%FUSION_HOME%" "%JAVAW%" -DSparkAgentVarDir="%FUSION_HOME%\var\spark-worker" ^
-classpath "%APP_DIR%\lib\*" com.lucidworks.spark.SparkAgent "%APP_DIR%\bin\spark-worker.bat" > "%LOG_DIR%\run.out"
...
@REM Get the PID of the spark-worker agent process
set AGENT_PID=0
For /F "Delims=" %%J In ('type "%FUSION_HOME%\var\spark-worker\spark-worker.pid"') do set AGENT_PID=%%~J
IF NOT "!AGENT_PID!"=="0" (
  taskkill /t /f /pid !AGENT_PID! > nul 2>&1
)
del "%FUSION_HOME%\var\spark-worker\spark-worker.pid"
